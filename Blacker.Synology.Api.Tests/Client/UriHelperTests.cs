...
namespace Blacker.Synology.Api.Tests.Client
{
    ...
    class UriHelperTests
    {
        ...
        public void BuildUriThrowsOnNullArgumentTest()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => UriHelper.BuildUri(new Uri("http://localhost/"), "path", null));
        }
        ...
        {
            Assert.That(UriHelper.BuildUri(baseUri, path, parameters).ToString(), Is.EqualTo(expected));
        }
        ...
        static IEnumerable<TestCaseData> BuildUriCreatesValidUriTestCaseData
        {
            get
            {
                ...
                yield return new TestCaseData(new Uri("http://localhost/"), "path", new Dictionary<string, object>() {{"query", "val&value"}}, "http://localhost/path?query=val%2526value");
